...
class NewEntity
  ...
  def data_set(args)
    if args
      @_data = IstoriesHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = IstoriesHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Array<New>, Array] the matching New items; raises IstoriesError on failure
