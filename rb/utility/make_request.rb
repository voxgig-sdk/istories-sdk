...
module IstoriesUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = IstoriesResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = IstoriesResponse.new(fetched)
    else
