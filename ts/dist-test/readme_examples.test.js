...
const TEST_SEED = { "entity": { "new": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+IstoriesSDK\b/.test(code) ||
        /\bIstoriesSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+IstoriesSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/IstoriesSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
