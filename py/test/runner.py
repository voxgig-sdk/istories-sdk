# Istories SDK test runner

from __future__ import annotations
import os
import json

from istories_sdk.utility.voxgig_struct import voxgig_struct as vs


class IstoriesTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            IstoriesTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = IstoriesTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = IstoriesTestRunner.getenv("ISTORIES_TEST_LIVE")
        override = IstoriesTestRunner.getenv("ISTORIES_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = IstoriesTestRunner.getenv(key)
        ...
        explain = IstoriesTestRunner.getenv("ISTORIES_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["ISTORIES_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if IstoriesTestRunner._test_control is not None:
            return IstoriesTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                IstoriesTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            IstoriesTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = IstoriesTestRunner.load_test_control()
...
def load_env_local():
    IstoriesTestRunner.load_env_local()


def env_override(m):
    return IstoriesTestRunner.env_override(m)


def entity_data(v):
    return IstoriesTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return IstoriesTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return IstoriesTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return IstoriesTestRunner.load_test_control()


def live_client_options():
    return IstoriesTestRunner.live_client_options()


def live_delay_ms():
    return IstoriesTestRunner.live_delay_ms()
