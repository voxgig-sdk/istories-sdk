...
boru-driven command-line client **and** interactive REPL for the Istories
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/istories-cli)
make build
...
./istories-cli list new

# 5. Override the API base URL for a single call
ISTORIES_BASE=https://api.example.com ./istories-cli list new
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/istories-cli
   ```
...
Run with no arguments to open a REPL (prompt `istories>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `new`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/istories-cli`. |
...
(`./istories-cli <expr>`) and interactively (the REPL), and expressions compose the
