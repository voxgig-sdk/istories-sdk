// Command Istories-cli is a CLI + REPL for the Istories SDK,
...
const prompt = "istories"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "new"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      ISTORIES_APIKEY (api key), ISTORIES_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: ISTORIES_APIKEY carries the API key and
	// ISTORIES_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("ISTORIES_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("ISTORIES_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewIstoriesSDK(opts)
