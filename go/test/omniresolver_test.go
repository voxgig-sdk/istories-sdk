...
import (
	...
	omni "github.com/voxgig-sdk/istories-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.IstoriesError); is {
				out := map[string]any{
					"name":    "IstoriesError",
...
func omniCtx(arg any, client *sdk.IstoriesSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.IstoriesSDK); is {
			client = live
