...
Istories API client, generated from the OpenAPI spec.

The Istories API provides access to documentary films and reports from Russia and beyond, focusing on investigations of military crimes and corruption.

Learn more about Istories API at [github.com/l0v3m0n3y/istories](https://github.com/l0v3m0n3y/istories).

This is an unofficial SDK for the Istories public API, generated by Voxgig with [`@voxgig/sdkgen`](https://github.com/voxgig/sdkgen). It is not affiliated with, endorsed by, or sponsored by the upstream API provider.
...
```bash
cd go-mcp && go build -o istories-mcp .
```
...
```json
{
  "mcpServers": {
    "istories": {
      "command": "/abs/path/to/istories-mcp"
    }
  }
}
```
...
| --- | --- | --- |
| **New** | The New entity (list). | `/page-data/news/page-data.json` |
...
$news = $client->New()->list();
print_r(array_map(fn($item) => $item->data_get(), $news));
```

### Golang

```go
import sdk "github.com/voxgig-sdk/istories-sdk/go"
...
- Upstream API: [https://github.com/l0v3m0n3y/istories](https://github.com/l0v3m0n3y/istories)
...
Generated from the Istories API OpenAPI spec by [@voxgig/sdkgen](https://github.com/voxgig/sdkgen).
