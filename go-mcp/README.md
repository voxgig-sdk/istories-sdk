...
[MCP](https://modelcontextprotocol.io) server exposing the Istories SDK as
two agent tools — `istories_list` and `istories_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/istories-mcp)
make build

# 2. Provide credentials via the environment
export ISTORIES_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user istories \
  -- /absolute/path/to/istories-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./istories-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// istories_list: first page of records
{ "entity": "new" }
{ "entity": "new", "query": { } }

// istories_load: one record by id
{ "entity": "new", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/istories-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user istories \
     -- "$PWD"/dist/*/istories-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `istories_list` and `istories_load` tools now appear
   in new sessions. Ask the agent to *"list new using istories"*
   and it calls `istories_list` with `{"entity":"new"}`.
...
```sh
export ISTORIES_APIKEY=sk_live_xxx            # API key
export ISTORIES_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `istories_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `istories_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `ISTORIES_APIKEY` | API key sent with every request. |
| `ISTORIES_BASE` | Optional override of the API base URL. |
...
```sh
./istories-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"istories_load","arguments":{"entity":"new","query":{"id":1}}}}'
```
