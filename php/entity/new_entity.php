...
class NewEntity
{
    ...
    {
        if ($args) {
            $this->_data = IstoriesHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = IstoriesHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   the SDK boundary; throws IstoriesError on failure (item-5 convention).
     */
