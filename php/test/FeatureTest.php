...
class FtCtrl extends IstoriesControl
{
...
class FtClient
{
    ...
    private IstoriesUtility $utility;

    public function __construct(IstoriesUtility $utility, array $options)
    {
    ...
    public function get_utility(): IstoriesUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (IstoriesContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public IstoriesContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (IstoriesContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = IstoriesConfig::make_feature($name);
    ...
    private static function buildUrl(IstoriesSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof IstoriesError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new IstoriesResponse($fetched);
        }
    ...
    private function fail(IstoriesContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (IstoriesContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
