...
class IstoriesMakeError
{
    public static function call(?IstoriesContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new IstoriesContext([], null);
        }
        $op = $ctx->op ?? new IstoriesOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new IstoriesResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof IstoriesError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new IstoriesError('', $msg, $ctx);
        ...
        if ($err instanceof IstoriesError) {
